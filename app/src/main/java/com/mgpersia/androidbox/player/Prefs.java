...
class Prefs {
    ...
    private void loadSavedPreferences() {
        ...
        try {
            orientation = Utils.Orientation.values()[mSharedPreferences.getInt(PREF_KEY_ORIENTATION, 1) - 1];

        } catch (Exception e) {
            if (Utils.Orientation.values().length > 0)
                orientation = Utils.Orientation.values()[Utils.Orientation.values().length - 1];
